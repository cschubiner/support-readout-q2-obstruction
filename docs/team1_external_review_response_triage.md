...
| `team1_finite_center_residue_theorem_note.md` or `team1_finite_center_residue_short_note.{tex,pdf}` | Response concerns the negative same-data finite-center residue. | Patch residue statement or comparison class first; avoid adding downstream physics claims. Rebuild the compact PDF when the theorem statement or demotion boundary changes. |
...
| `team1_split_tensor_factor_boundary_note.md` or `team1_split_tensor_factor_boundary_short_note.{tex,pdf}` | Response classifies the generated-algebra/tensor-factor boundary as known, false, or killed by a named split, type-I, factor, W*-tensor-product, or kernel-separation hypothesis. | Patch the note first; if false or already known, record the exact theorem and demote the split/tensor row before touching frozen artifacts. |
| `team1_probe_measurement_center_separation_note.md` or `team1_probe_measurement_center_separation_short_note.{tex,pdf}` | Response classifies the local probe-measurement center-separation boundary as known, false, or killed by a named Fewster-Verch/LCQFT/AQFT tomography, probe, effect, factor, or quotient hypothesis. | Patch the note first; if false or already known, record the exact theorem and demote the local measurement/probe row before touching frozen artifacts. |
...
2. Operator algebras/split inclusions: do generated-algebra correlations
   reconstruct tensor factors, or only modulo the multiplication kernel unless
   split/W*-tensor-product independence or kernel separation is supplied?
3. General operator-algebra quotient theory: is the finite-center descent
   package just central-projection quotient theory plus split/tensor exits?
4. Local probe measurements: do the induced Fewster-Verch probe effects
   separate the finite central contrasts, or can they still factor through a
   quotient/retraction?
5. DHR/DR sector reconstruction: does the supplied superselection structure
   retain finite central-character fibers, or only the restricted category?
6. OAQEC/JLMS/black-hole algebra: do recovery/relative entropy theorems imply
   maximality of the supplied algebra, or only recovery inside it?
7. Crossed-product entropy: does the gravitational Type II construction supply
   finite central trace/area calibration, or only entropy for a specified
   algebra and trace normalization?
8. Discrete black-hole spectroscopy: does a standard area-quantization or
...
9. Positive accountability: the finite accountability quotient is internally
