...
operator-algebra quantum-error-correction sense.  The cited OAQEC/JLMS
theorems are statements about this supplied algebra or entanglement-wedge
algebra; they are not, without an extra hypothesis, automatic maximality
statements for all hidden finite central refinements.  For every \(q\geq 2\), the
...
\paragraph{Primary-source checkpoint.}
Harlow's OAQEC theorem is formulated for a specified finite-dimensional
von Neumann algebra \(\M\) acting on the code, and its algebraic entropy
contains the classical distribution over the center of that supplied algebra.
Kamal--Penington extend the algebraic RT/OAQEC picture to boundary algebras
with centers and formulate reconstruction from a specified boundary algebra.
JLMS and Dong--Harlow--Wall supply the relative-entropy/reconstruction inputs
for the entanglement-wedge algebra under discussion.  The question here is
therefore not whether those theorems reconstruct a declared algebra; it is
whether reconstructability of that declared algebra also certifies maximality
as the full logical algebra against a larger finite central direct-sum
refinement.
...
algebra.  Throughout this note, \emph{declared} is the operative word:
reconstruction of \(\M\) means reconstruction of the algebra supplied to the
theorem.  It does not by itself assert that \(\M\) is the maximal logical
algebra compatible with all hidden finite central refinements.
...
Equivalently, in this finite test model,
\begin{align*}
  &\text{reconstructable supplied algebra } \M \\
  &\qquad\not\Rightarrow
    \text{ maximal full logical algebra, full hidden center, or } q=2 .
\end{align*}
The implication becomes valid only after a positive exit is added to the
hypotheses.
...
\begin{proposition}[central-sector entropy is a positive exit]
\label{prop:center-entropy}
Let \(\rho,\sigma\) be block-diagonal states on
\(\M_q\simeq\bigoplus_{j=1}^{q}\M\), written as
\[
  \rho=\bigoplus_{j=1}^{q} p_j\rho_j,\qquad
  \sigma=\bigoplus_{j=1}^{q} r_j\sigma_j,
\]
where \(p,r\) are probability vectors and \(\rho_j,\sigma_j\) are normalized
states on the \(j\)th copy of \(\M\).  If
\(\supp\rho\subseteq\supp\sigma\), then
\[
  S(\rho\|\sigma)
  =
...
\]
is the classical relative entropy of the central-sector distributions.
Consequently, a JLMS/OAQEC comparison that includes arbitrary central
probability vectors does not factor through \(Q_k\): it can detect changes in
the finite center through the classical term \(D(p\|r)\).
\end{proposition}

\begin{proof}
The logarithm of a block-diagonal positive operator is block diagonal on its
support:
\[
  \log\rho=\bigoplus_{j:p_j>0}(\log p_j+\log\rho_j),
  \qquad
  \log\sigma=\bigoplus_{j:r_j>0}(\log r_j+\log\sigma_j).
\]
Using \(S(\rho\|\sigma)=\Tr \rho(\log\rho-\log\sigma)\), the trace over the
direct sum separates into block traces:
\[
  S(\rho\|\sigma)
  =
  \sum_j p_j(\log p_j-\log r_j)
  +
  \sum_j p_j\,\Tr \rho_j(\log\rho_j-\log\sigma_j).
\]
This is the stated formula.  If the comparison class varies the central
probability vector, the first term varies, so the record is no longer
quotient-visible through a single selected summand.
\end{proof}
...
\begin{center}
\begin{tabular}{>{\raggedright\arraybackslash}p{0.35\linewidth}>{\raggedright\arraybackslash}p{0.51\linewidth}}
\toprule
Exit premise & Effect \\
\midrule
...
\(p_j\), then the data no longer factor through \(Q_k\).  Proposition
\ref{prop:center-entropy} is the relative-entropy version of this exit. \\
\addlinespace
...
This table is also the clean demotion rule.  If the OAQEC/JLMS theorem being
invoked already includes the full central-sector state family and a maximal
logical algebra as part of its hypotheses, Team 1's hidden-center comparison is
outside that setup.  The row should then be demoted to that named
maximality/full-center hypothesis, not presented as a new theorem.
